...
def rotate_tkinter_line(current_coords, pivot=None, angle=None):
    ...
    if (
        pivot == (new_x0, new_y0) or pivot == None
    ):  # TODO could probably also be handled as default parameter
...
def draw_rays():
    ...
    for key, val in drawing_to_internal_data_mapping.items():
        if isinstance(val, Ray):
            canvas.delete(key)

    # colors = ["yellow", "orange", "blue", "black", "red", "cyan", "magenta", "green"]
    colors = ["orange", "blue", "black", "red", "cyan", "magenta", "green"]
    color_i = 0
    for idx in range(0, len(Ray.rays), 6):
        for ray_num in range(6):
            this_ray = Ray.rays[ray_num + idx]
            this_id = canvas.create_line(
                get_draw_line_coords(this_ray), fill=colors[color_i], width=3
            )
...
def draw_all_room_entities():
    ...
    rs_box_diagonal_len = 20
    for source in Source.sources:
        this_id = canvas.create_rectangle(
            get_draw_rect_coords(source, rs_box_diagonal_len),
            fill="#00FF00",
            activeoutline="red",
        )
    ...
    for receiver in Receiver.receivers:
        this_id = canvas.create_rectangle(
            get_draw_rect_coords(receiver, rs_box_diagonal_len),
            fill="#FF0000",
            activeoutline="red",
        )
    ...
    for reflector in Reflector.reflectors:
        this_id = canvas.create_line(
            get_draw_line_coords(reflector), fill="purple", width=5, activefill="red"
        )
...
def open_file():
    ...
    filepath = askopenfilename(
        title="Open Pickle File...", filetypes=[("Pickle Files", "*.pickle")]
    )
    if not filepath:
        return
...
def update_drawing_reflector_after_rotation(internal_item):
    ...
    new_drawing_coords = (
        start_coords[0] * scale,
        start_coords[1] * scale,
        end_coords[0] * scale,
        end_coords[1] * scale,
    )
...
def on_rotate_clockwise_button():
    ...
    print(
        f"Angle: {round(math.degrees(cur_int_data_el.get_angle()) + 90, 2)} degrees"
    )  # +90 so horizontal is 0
...
def update_move_step():
    ...
    move_and_rotate_steps[0] = int(
        entry_box.get()
    )  # TODO update this so you can do fractional steps
    step_lable.configure(
        text=f"{move_and_rotate_steps[0]} (ft), {move_and_rotate_steps[1]} (deg)"
    )


def update_rotate_step():
    """updates the angle by which an reflector is rotated (also updates the label)"""
    move_and_rotate_steps[1] = int(
        entry_box.get()
    )  # TODO update this so you can do fractional angles??? (maybe not fractional degrees)
    step_lable.configure(
        text=f"{move_and_rotate_steps[0]} (ft), {move_and_rotate_steps[1]} (deg)"
    )
...
def user_draw_source(event):
    ...
    insert_at_coords = (
        canvas.canvasx(event.x) / scale,
        canvas.canvasy(event.y) / scale,
    )
...
def user_draw_start_reflector(event):
    ...
    this_id = canvas.create_line(
        start_at_coords, start_at_coords, fill="purple", width=5, activefill="red"
    )
...
def draw_image():  # TODO clean this up
    ...
    if len(images) != 0:
        ...
        room_size = (image_width / scale, image_height / scale)
        ...
        base_layer = canvas.create_image(
            0, 0, anchor="nw", tag="base_drawing", image=tk_image
        )
    else:
        images.clear()
        try:
    ...
    images.append(
        tk_image
    )  # you need to keep a reference to the tk_image or the garbage collector removes it
...
btn_draw_source = tkinter.Button(
    fr_buttons, text="Draw Source", command=bind_to_draw_source
)
btn_draw_reflector = tkinter.Button(
    fr_buttons, text="Draw Reflector", command=bind_to_draw_reflector
)
btn_draw_receiver = tkinter.Button(
    fr_buttons, text="Draw Receiver", command=bind_to_draw_receiver
)
spacer2 = tkinter.Label(fr_buttons, text="")
btn_move_active_obj_up = tkinter.Button(
    fr_buttons, text="Move Up", command=on_move_up_button
)
btn_move_active_obj_down = tkinter.Button(
    fr_buttons, text="Move Down", command=on_move_down_button
)
btn_move_active_obj_right = tkinter.Button(
    fr_buttons, text="Move Right", command=on_move_right_button
)
btn_move_active_obj_left = tkinter.Button(
    fr_buttons, text="Move Left", command=on_move_left_button
)
...
btn_rotate_active_obj_clockwise = tkinter.Button(
    fr_buttons, text="Rotate Clockwise", command=on_rotate_clockwise_button
)
btn_rotate_active_obj_counterclockwise = tkinter.Button(
    fr_buttons,
    text="Rotate Counterclockwise",
    command=on_rotate_counterclockwise_button,
)
btn_updt_move_step = tkinter.Button(
    fr_buttons, text="Update Move Step (ft)", command=update_move_step
)
btn_updt_angle_step = tkinter.Button(
    fr_buttons, text="Update Angle Step (deg)", command=update_rotate_step
)
entry_box = tkinter.Entry(fr_buttons, textvariable=5)
step_lable = tkinter.Label(
    fr_buttons,
    text=f"{move_and_rotate_steps[0]} (ft), {move_and_rotate_steps[1]} (deg)",
)
entry_box.insert(0, "input num val & click update")
entry_box.focus()
btn_selector = tkinter.Button(
    fr_buttons, text="Select", command=bind_to_element_selector
)
spacer4 = tkinter.Label(fr_buttons, text="")
btn_clear_canvas = tkinter.Button(fr_buttons, text="Clear Canvas", command=clear_canvas)
btn_delete_active_item = tkinter.Button(
    fr_buttons, text="Delete Active Item", command=delete_active_item
)
...
btn_rotate_active_obj_clockwise.grid(
    row=15, column=0, columnspan=6, sticky="ew", padx=5
)
btn_rotate_active_obj_counterclockwise.grid(
    row=16, column=0, columnspan=6, sticky="ew", padx=5
)
